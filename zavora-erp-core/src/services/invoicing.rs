...
pub async fn create_credit_note(
    ...
    journal_lines.push(CreateJournalLineRequest {
        account_code: engine.config().posting.accounts_receivable.clone(),
        debit: None,
    ...
    for line in &cn_lines {
        ...
        if line.vat_amount > Decimal::ZERO {
            journal_lines.push(CreateJournalLineRequest {
                account_code: engine.config().posting.vat_output.clone(),
...
async fn resolve_invoice_line(
    ...
    if let Some(product_id) = req.product_id {
        ...
        Ok(InvoiceLine {
            ...
            discount_percent: req.discount_percent.unwrap_or(Decimal::ZERO),
            account_code: req.account_code.clone().unwrap_or_else(|| engine.config().posting.default_sales.clone()),
...
pub async fn resolve_bill_line(
    ...
    if let Some(product_id) = req.product_id {
        ...
        let default_account = vendor
            ...
            .clone()
            .unwrap_or_else(|| engine.config().posting.default_expense.clone());
